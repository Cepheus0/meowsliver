...
- Accounts now have a real Postgres-backed runtime table and can act as the source of truth for net worth, asset allocation, liabilities, and snapshot-style investment views
...
- Accounts, asset buckets, and investment views now have a practical DB-backed snapshot layer even though they are not yet driven by a full position/lot ledger
...
| Use the accounts table as the current source of truth for balance-sheet style views | Solves the immediate empty-state problem across `/accounts`, dashboard, `/import`, and `/investments` without waiting for a full holdings ledger |
| Seed the current yellow-account snapshot into Postgres and store rough detail in account notes | Gives the user an immediately usable account database while preserving room to replace notes with a future lot-level model |
...
- Assets, liabilities, and investments are still snapshot-driven from the `accounts` table rather than a full holdings / valuation / transaction ledger
...
- Add an account-history / holdings ledger so cost basis, purchases, valuations, and gains stop depending on account notes or seeded metadata
