...
### Accounts Snapshot Recovery
- Ran the missing Drizzle migrations in the active localhost worktree so the `accounts` table exists in the local Postgres database again
- Added a reusable yellow-account catalog with current balances, aliases, seeded notes, and rough investment metadata derived from the provided screenshots
- Added `scripts/seed-yellow-accounts.ts` and used it to populate 18 live account records into Postgres for the currently running app
- Changed account hydration to retry on route changes instead of permanently giving up after the first failed `/api/accounts` request
- Connected account balances into derived asset, liability, and investment analytics so `/accounts`, dashboard, `/import`, and `/investments` now surface the same DB-backed snapshot
- Updated account detail notes to preserve multiline history text so screenshot-derived details are readable inside the app
- Verified the live UI with browser snapshots on `/accounts`, `/`, `/investments`, and `/import`
- Verified `typecheck`, all unit tests, and production `build`; `lint` passed with one pre-existing warning in `src/components/charts/CashflowChart.tsx`
...
| Asset / liability model | DB-backed snapshot working |
| Investment model | DB-backed snapshot working |
| Auth | not started |
...
4. Add a proper holdings / lot ledger for investment cost basis and valuation history.
